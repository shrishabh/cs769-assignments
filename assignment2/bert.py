...
class BertLayer(nn.Module):
    ...
    """
        ...
        # add-norm layer
        out_attn = self.add_norm(hidden_states, attn_value, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
        # feed forward
        out = self.interm_dense(out_attn)
        out = self.interm_af(out)
        # another add-norm layer

        out = self.add_norm(out_attn, out, self.out_dense, self.out_dropout, self.out_layer_norm)
